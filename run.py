    ...
    try:
        ...
        for chunk in random.sample(chunks, min(len(chunks), num_mcq * 2)):
            ...
            try:
                ...
                end_time = time.time()
                mcq_generation_times.append(end_time - start_time)
        ...
        short_q_generation_times = []
        short_a_generation_times = []
        ...
        for chunk in random.sample(chunks, min(len(chunks), num_short * 2)):
            ...
            try:
                ...
                short_q_generation_times.append(end_time_q - start_time_q)
                ...
                short_a_generation_times.append(end_time_a - start_time_a)
        ...
        if mcq_generation_times:
            avg_mcq_time = sum(mcq_generation_times) / len(mcq_generation_times)
            logger.info(f"Average MCQ generation time: {avg_mcq_time:.2f} seconds")

        if short_q_generation_times:
            avg_short_q_time = sum(short_q_generation_times) / len(short_q_generation_times)
            logger.info(f"Average short question generation time: {avg_short_q_time:.2f} seconds")

        if short_a_generation_times:
            avg_short_a_time = sum(short_a_generation_times) / len(short_a_generation_times)
            logger.info(f"Average short answer generation time: {avg_short_a_time:.2f} seconds")
